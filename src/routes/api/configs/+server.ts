...
import { getUserConfigs, getConfigId, aliasExists, countUserConfigs, createConfig } from '$lib/server/db';
...
export const GET: RequestHandler = async ({ platform, cookies, request }) => {
	...
	const results = await getUserConfigs(env.DB, user.id);

	const configs = results.map((config) => {
		let snapshot = null;
		if (config.snapshot) {
			try {
...
export const POST: RequestHandler = async ({ platform, cookies, request }) => {
	...
	const existing = await getConfigId(env.DB, user.id, slug);
	...
	if (alias) {
		...
		const aliasExistsRow = await aliasExists(env.DB, cleanAlias);
	...
	try {
		await createConfig(env.DB, {
			id,
			userId: user.id,
			slug,
			name,
			description: description || '',
			base_preset: base_preset || 'developer',
